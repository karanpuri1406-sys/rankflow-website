...
import type { CaseStudySummary } from '@/types/sanity'
...
export default async function CaseStudiesPage() {
  const studies: CaseStudySummary[] = await sanityFetch(allCaseStudiesQuery)
              ...
              <Link key={study._id} href={`/case-studies/${study.slug.current}`} style={{ textDecoration: 'none' }}>
                ...
                >
                  {study.coverImage?.asset?.url && (
                    <Image src={study.coverImage.asset.url} alt={study.title} width={680} height={200} style={{ width: '100%', height: '200px', objectFit: 'cover' }} />
                  )}
